...
### Added
- **Dual Binaries:** `cargo build` and `cargo install` now produce both `msc` and `firemusic` executables from `src/main.rs`.

### Changed
- **Install Scripts:** Linux, macOS, and Windows installers build with `--bins` and copy the real `msc` binary instead of duplicating `firemusic`.
