...
pub fn tactical_select(prompt: &str, items: &[String], is_multi: bool) -> Result<Option<Vec<usize>>> {
    enable_raw_mode()?;
    ...
    let res = loop {
        ...
        if items.len() > page_size {
            prompt_str = format!("{} (Page {}/{})", prompt_str, page + 1, (items.len() + page_size - 1) / page_size);
        }
        
        let safe_prompt = truncate(&prompt_str, term_width);
        queue!(stdout, crossterm::style::Print(format!("{}\n", safe_prompt.yellow().bold())))?;

        for idx in start_idx..end_idx {
            let is_hovered = idx == hovered;
            let is_selected = selected.contains(&idx);
            
            let prefix = if is_hovered { ">".cyan().bold() } else { " ".white() };
            let check = if is_multi {
                if is_selected { "[x]".cyan() } else { "[ ]".dark_grey() }
            } else {
                "   ".white()
            };
            ...
            if let CEvent::Key(key) = event::read()? {
                ...
                match key.code {
                    ...
                    KeyCode::Up | KeyCode::Char('k') => {
                        if hovered > 0 { hovered -= 1; } else { hovered = items.len() - 1; }
                    }
                    KeyCode::Down | KeyCode::Char('j') => {
                        if hovered < items.len() - 1 { hovered += 1; } else { hovered = 0; }
                    }
                    KeyCode::Left | KeyCode::Char('h') => {
                        if items.len() > page_size {
                            let current_page = hovered / page_size;
                            if current_page > 0 { hovered = (current_page - 1) * page_size; }
                            else { hovered = ((items.len() - 1) / page_size) * page_size; }
                        }
                    }
                    KeyCode::Right | KeyCode::Char('l') => {
                        if items.len() > page_size {
                            ...
                            if current_page < last_page { hovered = (current_page + 1) * page_size; }
                            else { hovered = 0; }
                        }
                    }
                    KeyCode::Char(' ') => {
                    ...
                    KeyCode::Enter => {
                        ...
                        for idx in start_idx..end_idx {
                            ...
                            let check = if is_multi {
                                ...
                                if is_selected { "[x]".cyan() } else { "   ".white() }
                            };
                            let item_styled = if is_hovered { truncate(&items[idx], term_width.saturating_sub(6)).white().bold() } 
                                             else { truncate(&items[idx], term_width.saturating_sub(6)).dark_grey() };
                            queue!(stdout, cursor::MoveToColumn(0), crossterm::style::Print(format!("{} {} {}\n", prefix, check, item_styled)))?;
                        }
                        stdout.flush()?;
    ...
    if items.len() > page_size { lines_to_move_down += 2; }
    
    execute!(stdout, cursor::MoveDown(lines_to_move_down as u16), cursor::MoveToColumn(0))?;
